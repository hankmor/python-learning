...
1、MUA：Mail User Agent, 邮件用户代理，就是用户使用的邮箱客户端软件，比如 foxmail 客户端
2、MTA：Mail Transfer Agent，邮件传输代理，即邮箱服务提供商，比如网易，gmail
3、MDA：Mail Delivery Agent，邮件投递代理，长期保存最终目的邮件的服务器，收件人最后通过 MUA 直接从 MDA 下载邮件就完成邮件的收件
"""
import smtplib
